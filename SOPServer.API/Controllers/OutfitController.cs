...
namespace SOPServer.API.Controllers
{
    ...
    public class OutfitController : BaseController
    {
        ...
        /// <summary>
        /// Batch Virtual try-on feature - Apply clothing items to multiple human images using AI
        /// </summary>
        /// <remarks>
        /// **Roles:** USER, STYLIST, ADMIN
        ///
        /// **Request Body:**
        /// - `items`: Array of try-on items, each containing:
        ///   - `uuid`: Integer identifier generated by frontend (required)
        ///   - `humanImageUrl`: URL of the human image (required)
        ///   - `clothingUrls`: Array of clothing item image URLs to apply (required, at least 1 item)
        ///
        /// **Response:**
        /// - `totalTime`: Total time taken to process all items
        /// - `results`: Array of results for each item:
        ///   - `uuid`: Matching identifier from request
        ///   - `success`: Boolean indicating if try-on was successful
        ///   - `url`: URL of the generated try-on image (if successful)
        ///   - `error`: Error message (if failed)
        ///   - `time`: Time taken to process this item
        ///
        /// **Note:** This feature processes multiple try-on requests in parallel for better performance.
        /// All processing times and errors are logged to the console.
        /// </remarks>
        [HttpPost("batch-virtual-try-on")]
        public Task<IActionResult> BatchVirtualTryOn([FromBody] BatchVirtualTryOnRequest model)
        {
            return ValidateAndExecute(async () => await _outfitService.BatchVirtualTryOn(model));
        }
    }
}
